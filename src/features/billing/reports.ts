...
export async function financialReport(filters: {
  category?: string;
  customerId?: string;
  from?: string;
  status?: string;
  ...
  const included = source.filter(
    ({ invoice }) =>
      !filters.category || invoice.lines.some((line) => line.category === filters.category),
  );
  const rows: ReportRow[] = included.map(({ customer, invoice }) => ({
  ...
  const creditRows = await db
    .select({ credit: credits, customer: customers.name, source: invoices })
    .from(credits)
    ...
    .where(
      and(
        ...
        ...(filters.from ? [gte(credits.createdAt, new Date(`${filters.from}T00:00:00Z`))] : []),
        ...(filters.to ? [lte(credits.createdAt, new Date(`${filters.to}T23:59:59.999Z`))] : []),
      ),
    )
  ...
  for (const { credit, customer, source } of creditRows) {
    if (
      filters.status ||
      (filters.category && !source.lines.some((line) => line.category === filters.category))
    )
      continue;
    rows.push({
      category: "Credit",
